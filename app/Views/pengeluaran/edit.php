...
<form method="post" class="container mt-4 p-5 border border-1 rounded" id="form" x-data x-effect="setNotification()">
  <div class=""></div>
  <h5 class="text-center mb-5">Form Edit Pengeluaran</h5>
  <div class="mb-3">
  ...
  <div class="d-grid gap-2">
    ...
    <button class="btn btn-primary" type="button" @click="location.reload()">Reset</button>
  </div>
</form>
<?= $this->endSection() ?>
...
<script>
  ...
  document.addEventListener('alpine:init', () => {
    Alpine.store('global', {
      'form': {
        'id_label': <?= $dataPengeluaran['id_label'] ?>,
  ...
  async function loadSelectLabel() {
    ...
    try {
      ...
      this.$nextTick(() => {
        storeGlobal.form.id_label = <?= $dataPengeluaran['id_label'] ?>;
      })
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await axios.put('/api/pengeluaran/<?= $idPengeluaran ?>', data);
