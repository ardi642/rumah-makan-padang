...
<div class="container mt-5 p-5 border border-1 rounded">
  <h5 class="text-center mb-4">Tabel Label Pengeluaran</h5>
  <div class="text-center mb-5">
    <a class="btn btn-success" href="/LabelPengeluaran/tambah" role="button">
      Tambah Label Pengeluaran
    </a>
  </div>
...
<script>
  ...
  let tabel = $('#tabel').DataTable({
    ...
    columns: [{
      ...
      },
      {
        ...
        render: function(data, type, row, meta) {
          return `
            <a href="LabelPengeluaran/edit/${row.id_label}" class="btn btn-sm btn-warning">Edit</a>
    ...
    ajax: {
      url: '/api/LabelPengeluaran/selectDatatable'
    }
  });
  ...
  $('#tabel').on('click', '.tombol-hapus', async function(e) {
    ...
    let keputusan = await Swal.fire({
      ...
      text: `Anda akan menghapus label pengeluaran ${rowData.label} ?`,
      icon: 'warning',
    ...
    try {
      const response = await axios.delete(`/api/LabelPengeluaran/${rowData.id_label}`);
      status = 'sukses';
    } catch (error) {
      console.log(error);
      ...
      if (status == 'sukses') {
        ...
        Toast.fire({
          icon: 'success',
          title: `label pengeluaran ${rowData.label} berhasil dihapus`
        })
      ...
      if (status == 'gagal') {
        Toast.fire({
          icon: 'error',
          title: `label pengeluaran ${rowData.label} gagal dihapus`
        })
      }
    }
  })
