...
<div class="container mt-4 text-center">
  <a class="btn btn-primary" href="/LabelPengeluaran" role="button">Kembali</a>
</div>
<form method="post" class="container mt-4 p-5 border border-1 rounded" id="form" x-data x-effect="setNotification()">
  <h5 class="text-center mb-5">Form Edit Label Pengeluaran</h5>
  <div class="mb-3">
    <label class="form-label">label pengeluaran</label>
    <input type="text" class="form-control" name="label" placeholder="masukkan label pengeluaran" :value="$store.form.label" x-model="$store.form.label" @keyup="resetValidasi('label')">
...
<script>
  ...
  function setNotification() {
    ...
    if (status == 'sukses') {
      Toast.fire({
        icon: 'success',
        title: 'label pengeluaran berhasil diperbarui'
      })
    }
    ...
    if (status == 'gagal') {
      Toast.fire({
        icon: 'error',
        title: 'label pengeluaran gagal diperbarui'
      })
    }
  ...
  async function loadDataForm() {
    ...
    try {
      ...
      const response = await axios.get('/api/LabelPengeluaran/find/<?= $idPengeluaran ?>');
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await axios.put('/api/LabelPengeluaran/<?= $idPengeluaran ?>', data);
