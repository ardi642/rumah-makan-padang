...
<form method="post" class="container mt-4 p-5 border border-1 rounded" id="form" x-data x-effect="setNotification()">
  <div class=""></div>
  <h5 class="text-center mb-5">Form Tambah Label Pengeluaran</h5>
  <div class="mb-3">
...
<script>
  ...
  function setNotification() {
    ...
    if (status == 'sukses') {
      ...
      Toast.fire({
        icon: 'success',
        title: 'label pengeluaran berhasil ditambahkan'
      })
    }
    ...
    if (status == 'gagal') {
      Toast.fire({
        icon: 'error',
        title: 'label pengeluaran gagal ditambahkan'
      })
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await axios.post('/api/LabelPengeluaran', data);
